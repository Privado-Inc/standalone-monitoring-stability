        ...
        try:
            ...
            if use_docker:
                ...
                if generate_unique_flow:
                    first_command = f'cd {cwd}/temp/binary/{first_branch}/bin && ./privado-core scan {scan_dir} -ic {cwd}/temp/privado --skip-upload --test-output -Dlog4j.configurationFile={cwd}/temp/log-rule/{first_branch}/log4j2.xml | tee {cwd}/temp/result/{first_branch}/{repo}-output.txt'
                else:
                    first_command = f'cd {cwd}/temp/binary/{first_branch}/bin && ./privado-core scan {scan_dir} -ic {cwd}/temp/privado --skip-upload -Dlog4j.configurationFile={cwd}/temp/log-rule/{first_branch}/log4j2.xml | tee {cwd}/temp/result/{first_branch}/{repo}-output.txt'
            ...
            if use_docker:
                ...
                if generate_unique_flow:
                    second_command = f'cd {cwd}/temp/binary/{second_branch}/bin && ./privado-core scan {scan_dir} -ic {cwd}/temp/privado --skip-upload --test-output -Dlog4j.configurationFile={cwd}/temp/log-rule/{second_branch}/log4j2.xml | tee {cwd}/temp/result/{second_branch}/{repo}-output.txt'
                else:
                    second_command = f'cd {cwd}/temp/binary/{second_branch}/bin && ./privado-core scan {scan_dir} -ic {cwd}/temp/privado --skip-upload -Dlog4j.configurationFile={cwd}/temp/log-rule/{second_branch}/log4j2.xml | tee {cwd}/temp/result/{second_branch}/{repo}-output.txt'
