...
def build(skip_build = False):
    ...
    build_binary_and_move(repo_second, config.HEAD_CORE_BRANCH_NAME, config.HEAD_CORE_BRANCH_KEY, f'{os.getcwd()}/temp/david-privado-core')
    move_log_rule_file(f'{pwd}/temp/privado-core/log4j2.xml', config.HEAD_CORE_BRANCH_KEY)


def build_binary_and_move(repo, branch_name, branch_file_name, core_dir):
    path = os.getcwd()
    ...
    print(f"Binary built at {core_dir} and moved from {binary_dir} to {final_dir}")
