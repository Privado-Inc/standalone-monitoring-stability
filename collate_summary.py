...
class ScanFailureReport():
    ...
    def get_number_repos_failed(self, language_summary):
        for row in language_summary[self.start:self.start + 3]:
...
def main():
    ...
    put_joern_summary = True

    for language_summary in get_file_contents(args.summary_dir):
        scantime_start = get_num_until_summary_start(language_summary)

        if args.joern_update and put_joern_summary:
            joern_version.get_result(language_summary)
            summary += joern_version.get_summary()
            put_joern_summary = False
